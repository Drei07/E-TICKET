...
public function mainUrl(){
 ...
 public function register($first_name, $middle_name, $last_name, $phone_number, $position, $email, $hash_password, $tokencode, $user_type)
 {
  try
  {       
   $password = md5($hash_password);
   $stmt = $this->conn->prepare("INSERT INTO users(first_name, middle_name, last_name, phone_number, position, email, password, tokencode, user_type) 
                                        VALUES(:first_name, :middle_name, :last_name, :phone_number, :position, :email, :password, :tokencode, :user_type)");
   ...
   if($stmt->rowCount() == 1)
   {
    if($userRow['status']=="Y")
    {
     if($userRow['password']==md5($hash_password))
     {
      ...
      $_SESSION['employerSession'] = $userRow['id'];
      return true;
     }
     else
     {
      ...
      header("Location: ../../../private/employer/");
      exit;
     }
    }
    else
 ...
 {
  if(isset($_SESSION['employerSession']))
  {
   return true;
  }
 }
 ...
 {
  unset($_SESSION['employerSession']);
 }
