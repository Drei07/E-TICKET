...
class adminRegistration {
    ...
    // add sub-admin
    public function registerAdmin($first_name, $middle_name, $last_name, $phone_number, $email, $hash_password, $tokencode, $user_type)
    {
        $stmt = $this->admin->runQuery("SELECT * FROM users WHERE email=:email");
        $stmt->execute(array(":email"=>$email));
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if ($stmt->rowCount() > 0) {
            ...
            if ($this->admin->register($first_name, $middle_name, $last_name, $phone_number, $email, $hash_password, $tokencode, $user_type)) {
                $id = $this->admin->lasdID();
                $key = base64_encode($id);
                $id = $key;
                ...
                <body>
                    <div class='container'>
                        ...
                        Password:<br />$hash_password
                        <p>To complete your account registration, please click the button below to verify your email address:</p>
                        <p><a class='button' href='$this->main_url/private/admin/verify-account?id=$id&code=$tokencode'>Verify Email</a></p>
                        <p>If you did not sign up for an account, you can safely ignore this email.</p>
                        <p>Thank you!</p>
                    </div>
                </body>
    ...
    //edit sub-admin
    public function editAdmin($user_id, $first_name, $middle_name, $last_name, $phone_number) {

        $stmt = $this->admin->runQuery('SELECT first_name, middle_name, last_name, phone_number FROM users WHERE id=:id');
        $stmt->execute(array(':id' => $user_id));
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        $stmt = $this->admin->runQuery('UPDATE users SET first_name=:first_name, middle_name=:middle_name, last_name=:last_name, phone_number=:phone_number WHERE id=:id');
        $exec = $stmt->execute(array(
            ':id'             => $user_id,
            ':first_name'     => $first_name,
            ':middle_name'    => $middle_name,
        ...
        if ($exec) {
            $_SESSION['status_title'] = 'Success!';
            $_SESSION['status'] = 'Admin information successfully updated!';
            $_SESSION['status_code'] = 'success';
            $_SESSION['status_timer'] = 40000;
        } else {
            $_SESSION['status_title'] = 'Oops!';
    ...
    //disabled sub-admin
    public function disabledAdmin($admin_id){
        ...
        if ($exec) {
            $_SESSION['status_title'] = 'Success!';
            $_SESSION['status'] = 'Admin successfully disabled!';
            $_SESSION['status_code'] = 'success';
            $_SESSION['status_timer'] = 40000;
        } else {
            $_SESSION['status_title'] = 'Oops!';
    ...
    public function activateAdmin($admin_id){
        ...
        if ($exec) {
            $_SESSION['status_title'] = 'Success!';
            $_SESSION['status'] = 'Admin successfully activated!';
            $_SESSION['status_code'] = 'success';
            $_SESSION['status_timer'] = 40000;
        } else {
            $_SESSION['status_title'] = 'Oops!';
...
if(isset($_POST['btn-add-admin'])) {
    ...
    $adminRegistration->registerAdmin($first_name, $middle_name, $last_name, $phone_number, $email, $hash_password, $tokencode, $user_type);
}
...
if(isset($_POST['btn-edit-admin'])) {
    ...
    $adminUpdate = new adminRegistration();
    $adminUpdate->editAdmin($user_id, $first_name, $middle_name, $last_name, $phone_number, $department);
}

//disabled sub-admin
if (isset($_GET['disabled_admin'])) {
    $admin_id = $_GET["id"];

    $disabled_admin = new adminRegistration();
    $disabled_admin->disabledAdmin($admin_id);
}
...
if (isset($_GET['activate_admin'])) {
    ...
    $activate_admin = new adminRegistration();
    $activate_admin->activateAdmin($admin_id);
}
?>