...
$statement->execute(array(":user_type" => 1));
...
';
  while($row=$statement->fetch(PDO::FETCH_ASSOC))
  {
    ...
    if ($row["account_status"] == "active") {
      $button = '<button type="button" class="btn btn-danger V"><a href="controller/user-controller?id='.$row["id"].'&disabled_admin=1" class="delete"><i class="bx bxs-trash"></i></a></button>';
      $status = '<button type="button" class="btn btn-success V" style="width: 80px;">Active</button>';
    
    } else if ($row["account_status"] == "disabled") {
      $button = '<button type="button" class="btn btn-warning V"><a href="controller/user-controller?id='.$row["id"].'&activate_admin=1" class="activate">Activate</a></button>';
