	...
	<section id="content">
		...
		<main>
					...
					<div class="details">
						...
						?>
							...
							<div class="action">
								...
								<button type="button" class="btn2 btn-primary"><a href="controller/access-token-controller.php?event_id=<?php echo $eventId ?>&print_access_tokens_optional=1" class="print"><i class='bx bxs-printer'></i> Print Access Token</a></button>
							</div>
					</div>
				</li>
			</ul>
			...
			<div class="table-data">
				<div class="order">
					<div class="head">
						<h3><i class='bx bxs-user-detail'></i> List of Printed Access Tokens</h3>
					</div>
					<!-- BODY -->
							...
							<section class="data-form-modals">
								<div class="registration">
									<form action="controller/access-token-controller.php?user_id=<?php echo $user_id ?>&event_id=<?php echo $eventId ?>" method="POST" class="row gx-5 needs-validation" name="form" onsubmit="return validate()" novalidate style="overflow: hidden;">
										...
										<div class="addBtn">
											<button type="submit" class="btn-dark" name="btn-add-access-token-optional" id="btn-add" onclick="return IsEmpty(); sexEmpty();">Generate</button>
										</div>
									</form>
								</div>
							</section>
	...
	<script>
		...
		$(document).ready(function() {
			...
			function load_data(page, query = '') {
				$.ajax({
					url: "tables/pdf-table.php?event_id=<?php echo $eventId ?>",
					method: "POST",
					data: {
						page: page,
						query: query
